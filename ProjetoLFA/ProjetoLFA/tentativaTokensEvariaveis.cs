...
namespace ProjetoLFA
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //==== Colocar epsilon transição nos tokens, para reconhecer variaveis
            foreach(string token in tokens)
            {
                string nomeRegra = "S";
                for (int i=0; i<token.Length-1; i++)
                {
                    ...
                    foreach (var regra in regras)
                    {
                        if (!regra.nomeRegra.Equals(nomeRegra)) continue;
                        foreach (var trans in regra.transicoes)
                        {
                            if (trans.simbolo == token[i])
                            {
                                achouToken = 1;
                                nomeRegra = trans.regraTransicao;
                                regraAtual = regra;
                                break;
                            }
                        }
                    ...
                    int addTransicao = 1;
                    foreach (var trans in regraAtual.transicoes)
                    {
                        if (trans.simbolo == token[i] && trans.regraTransicao.Equals("")){
                            addTransicao = 0;
                        }
                    }
                    if (addTransicao == 1)
                    {
                        //Adicionar producao com terminal para reconhecer variavel
                        ItemRegra itemRegra = new ItemRegra();
                        itemRegra.regraTransicao = "";
                        itemRegra.simbolo = token[i];
                        regraAtual.transicoes.Add(itemRegra);
                        break;
                    }
                }
            }
